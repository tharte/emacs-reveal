...
#
# First, revealimg() expects up to four arguments, a filename for a
# file with meta-data about an image and up to three optional
# arguments: a caption, the maximum height of image in HTML, e.g.,
# 50vh, and a list of CSS classes (space separated, containing "figure").
# This is meant to be used with horizontally centered images, caption
# underneath, license text rotated to the right.
# Second, reveallicense() expects up to three arguments, the filename
# for meta-data and optionally the maximum height and CSS classes.
...
# reveal-config.el.
#
#+MACRO: revealimg (eval (reveal-export-attribution $1 $2 $3 $4))
#+MACRO: reveallicense (eval (reveal-export-attribution $1 nil $2 $3))
