...
# Third, revealsamelicense expects up to three arguments (the first
# three of reveallicense) and can be used when the license for the image
# agrees with the one of the document and, thus, does not need to be
# repeated.
...
#+MACRO: revealsamelicense (eval (reveal-export-attribution $1 nil (if (= 0 (length "$2")) nil $2) (if (= 0 (length "$3")) nil $3) 'none))
